...
const addBalance = async (req, res) => {
    ...
    if (!type || typeof type !== "string" || !BalanceAccount_1.BALANCE_ACCOUNT_TYPES.includes(type)) {
...
const updateBalance = async (req, res) => {
    ...
    if (!id) {
        return res.status(400).json({ success: false, message: "Balance source id is required." });
    }
    if (typeof amount !== "number" || Number.isNaN(amount) || amount < 0) {
        return res.status(400).json({
            success: false,
...
const getCategories = async (req, res) => {
    ...
    try {
        const categories = await Category_1.default.find({ userId: auth.userId }).sort({ name: 1 });
...
const deleteCategory = async (req, res) => {
    ...
    try {
        ...
        const expenseCount = await Expense_1.default.countDocuments({ userId: auth.userId, category: normalized });
        if (expenseCount > 0) {
        ...
        const deleted = await Category_1.default.findOneAndDelete({ userId: auth.userId, name: normalized });
        if (!deleted) {
...
const addExpense = async (req, res) => {
    ...
    if (!note?.trim()) {
        return res.status(400).json({ success: false, message: "Expense note is required." });
    }
    ...
    try {
        ...
        const existingCategory = await Category_1.default.findOne({ userId: auth.userId, name: normalizedCategory });
        if (!existingCategory) {
...
const getExpensesList = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({ success: true, data: result.expenses, pagination: result.pagination });
    }
    catch (error) {
...
const addSalaryEntry = async (req, res) => {
    ...
    if (typeof amount !== "number" || Number.isNaN(amount) || amount <= 0) {
        return res.status(400).json({ success: false, message: "A valid salary amount is required." });
    }
    const salaryDate = date ? new Date(date) : new Date();
    if (Number.isNaN(salaryDate.getTime())) {
        return res.status(400).json({ success: false, message: "Salary date is invalid." });
    }
    try {
        const salary = await (0, financeService_1.addSalary)(auth.userId, amount, salaryDate);
        return res.status(201).json({ success: true, message: "Salary added successfully.", data: salary });
    }
    catch (error) {
        return res.status(400).json({ success: false, message: getErrorMessage(error) });
    }
...
const createLoanEntry = async (req, res) => {
    ...
    if (typeof amount !== "number" || Number.isNaN(amount) || amount <= 0) {
        return res.status(400).json({ success: false, message: "A valid loan amount is required." });
    }
    if (!sourceType || !["PERSONAL", "BORROWED"].includes(sourceType)) {
        return res.status(400).json({ success: false, message: "sourceType must be PERSONAL or BORROWED." });
    }
    try {
...
const repayLoanEntry = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({ success: true, message: "Loan repayment recorded successfully.", data: loan });
    }
    catch (error) {
        return res.status(400).json({ success: false, message: getErrorMessage(error) });
    }
...
const getLoanList = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({ success: true, data: result.loans, pagination: result.pagination });
    }
    catch (error) {
        return res.status(500).json({ success: false, message: getErrorMessage(error) });
    }
