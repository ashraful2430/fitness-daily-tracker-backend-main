...
const getMonthlyOverview = async (req, res) => {
    try {
        ...
        const availableBalanceEndOfMonth = !hasMoneyActivity
            ? 0
...
async function getMonthlyDailySeries(userId, start, end) {
    ...
    const [incomeAgg, expenseAgg, workoutAgg, focusAgg, learningAgg, scoreMap] = await Promise.all([
        Income_1.default.aggregate([{ $match: { userId, date: { $gte: start, $lt: end } } }, { $group: { _id: { $dateToString: { format: "%Y-%m-%d", date: "$date", timezone: SERVER_TIMEZONE } }, total: { $sum: "$amount" } } }]),
        Expense_1.default.aggregate([{ $match: { userId, date: { $gte: start, $lt: end } } }, { $group: { _id: { $dateToString: { format: "%Y-%m-%d", date: "$date", timezone: SERVER_TIMEZONE } }, total: { $sum: "$amount" } } }]),
        Workout_1.default.aggregate([{ $match: { userId: objectUserId, createdAt: { $gte: start, $lt: end } } }, { $group: { _id: { $dateToString: { format: "%Y-%m-%d", date: "$createdAt", timezone: SERVER_TIMEZONE } }, total: { $sum: 1 } } }]),
        DashboardData_1.FocusSession.aggregate([{ $match: { userId: objectUserId, date: { $gte: start, $lt: end } } }, { $group: { _id: { $dateToString: { format: "%Y-%m-%d", date: "$date", timezone: SERVER_TIMEZONE } }, total: { $sum: "$totalMinutes" } } }]),
        LearningSession_1.default.aggregate([
            { $match: { userId, studyDate: { $gte: toDateKey(start), $lte: toDateKey(addDays(end, -1)) } } },
            {
                $group: {
...
async function getMonthlyDailyScores(userId, start, end) {
    const objectUserId = new mongoose_1.default.Types.ObjectId(userId);
    const lastLogin = await User_1.default.findById(userId).select("lastLoginDate").lean();
    ...
    const [workoutAgg, focusAgg, learningAgg, sectionsAgg] = await Promise.all([
        ...
        ScoreSection_1.ScoreSection.aggregate([
            { $match: { userId: objectUserId, date: { $gte: start, $lt: end } } },
            {
                $group: {
    ...
    for (const dateKey of keys) {
        const score = (0, dashboardProgressService_1.computeDailyProgress)({
    ...
    return {
        byDate,
        averageDailyScore: values.length > 0
